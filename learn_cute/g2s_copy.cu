...
__global__ void initBuffer(T* ptr, int size) {
    ...
    float v = (float)(ptr[thr_idx]);
    // printf("thread_idx: %d, ptr[%d] = %f\n", thr_idx, thr_idx, v);
}
...
__global__ void g2rCopy(T* ptr) {
    ...
    // Just copy once
    copy(g2s_tiled_copy, thr_g(_, 1, 0), fragment(_, 0, 0));
#ifdef DEBUG
    int size = fragment.size();
    int idx0_size = fragment(_, 0, 0).size();
    int idx1_size = fragment(0, _, 0).size();
    int idx2_size = fragment(0, 0, _).size();
    printf("Thread ID: %d hold A value %.1f, %.1f, %.1f, %.1f %d %d %d %d\n",
        idx, float(fragment(0)), float(fragment(1)), float(fragment(2)), float(fragment(3)), size, idx0_size, idx1_size, idx2_size);
#endif
}
...
int main(void) {
    using namespace cute;
    float* dev_p = nullptr;
    initBufferKernelLaunch<float>(&dev_p);
    if (!dev_p) {
